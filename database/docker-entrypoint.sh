...
# This is an exact copy of the docker-entrypoint.sh script in the official MySQL docker container, taken from:
# https://github.com/docker-library/mysql/blob/d284e15821ac64b6eda1b146775bf4b6f4844077/5.7/docker-entrypoint.sh
# The small change is located in lines 68/69.
# It processes a bash script that creates a SQL file with information taken from environment variables passed
# from docker-compose.yml. This SQL is then used to initialize the database.
...
#			*.sh)     mysql_note "$0: running $f"; . "$f" ;;
			*.sh)     mysql_note "$0: running $f"; ./$f; docker_process_sql < "$f.sql"; echo;;
