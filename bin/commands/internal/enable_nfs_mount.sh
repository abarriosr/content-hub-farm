...
# Enables native NFS on macOS for Docker

# Adapted from https://raw.githubusercontent.com/drud/ddev/master/scripts/macos_ddev_nfs_setup.sh
...
mkdir -p ~/.chf
docker run --rm -t -v /$HOME/.chf:/tmp/junker99 busybox:latest ls //tmp/junker99 >/dev/null || ( echo "Docker does not seem to be running or functional, please check it for problems" && exit 103)
